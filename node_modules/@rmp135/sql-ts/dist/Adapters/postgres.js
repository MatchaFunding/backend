...
export default {
    ...
    `;
        const ungroupedEnums = (await db.raw(sql, { schemas: config.schemas })).rows;
        const groupedEnums = uniqBy(ungroupedEnums, e => `${e.name}.${e.schema}`)
            .map(row => ({
            name: row.name,
            schema: row.schema,
    ...
    `;
        const results = await db.raw(sql, { schemas });
        return results.rows;
    },
